      ...
      if (loaderUtils.isUrlRequest(url, root)) {
        ...
        if (compilationMpx.processingSubPackages) {
          for (let src in subPackagesMap) {
            ...
            if (requestPath.startsWith(src) && !mainResourceMap[requestPath]) {
        ...
        let relativePath = toPosix(path.relative(path.dirname(originFilePath), path.resolve(projectRoot, outputFilename)))
        if (mode === 'swan') {
